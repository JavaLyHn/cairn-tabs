...
describe('MERGE_DUPLICATES(F-05)', () => {
  it('关闭冗余标签、保留最新打开的,记录同步清除', async () => {
    ...
    expect(after.tabs.map((t) => t.id).toSorted()).toEqual(
      [keeperRec.id, after.tabs.find((t) => t.url === 'https://x.com/unique')!.id].toSorted(),
    );
  });
});
...
describe('加载期并发不产生重复记录(回归 Bug:开一个网站出现三条)', () => {
  it('onCreated 与加载期 onUpdated 并发 → 同一 chromeTabId 只建一条记录', async () => {
    ...
    await Promise.all([
      ...
      fake.onUpdated.emit(
        id,
        { title: 'ElevateSphere AIG' },
        { ...tab, title: 'ElevateSphere AIG' },
      ),
...
describe('CLOSE_TAB:标签导航后仍能关闭(回归 Bug:× 关不掉)', () => {
  it('记录 url 陈旧(标签已导航)→ 点 × 仍真正关掉标签', async () => {
    const id = await fake.userOpenTab('https://a.com/1', { title: 'A' });
    const [rid] = await inboxTabIds();
    // 标签导航到新 url,但记录还是旧 url(模拟漏收 onUpdated 的窗口 / SPA 内跳转)
    fake.tabsById.get(id)!.url = 'https://a.com/2';

    await handleCommand({ type: 'CLOSE_TAB', tabRecordId: rid! }, ctx);

    expect(fake.tabsById.has(id)).toBe(false); // 真实标签被关闭,而非只清记录
...
describe('自动聚簇(F-07)', () => {
  ...
  it('从命名簇内标签点开的新标签,经原生分组归入该簇', async () => {
    // 先手动建簇并放一个标签(建立原生分组)
    await fake.userOpenTab('https://github.com/a/b', { title: 'repo' });
    await handleCommand({ type: 'CREATE_CONTEXT', name: 'work' }, ctx);
