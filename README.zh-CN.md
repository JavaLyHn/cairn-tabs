...
**面向程序员的标签页上下文管理器。** 自动把浏览器标签按任务归类,一键归档、恢复整个任务,全局秒搜。Chrome / Edge 侧边栏插件 —— 本地优先、无账号。
...
> **cairn** 是山道上用石块垒成的路标 —— 为你散乱的标签立个路标。
...
- **手动任务** —— 创建 / 重命名 / 删除;拖拽标签在任务间归属。
- **整任务归档 / 恢复** —— 一键归档并关闭、一键限速恢复,5 秒可撤销。
- **AI 整理全部** —— 头部「✦ 整理全部」按钮把**当前所有打开的标签**一次重新聚类,跨组移动以尽量不留散标签。**加星与手动摆放的标签保持不动**;空组自动清理;预览标注每个标签的原组,整个整理**一键可撤销**。(「未分类」另有更保守的「✦ AI 整理」,见下。)
- **会话恢复** —— 任务能扛住浏览器重启。重启后标签按 URL 重绑回原记录、任务按分组标题重连回原生分组,数据不丢。若某任务的标签 Chrome 完全没恢复,该任务会被**自动归档**(保留 URL),一键即可恢复回来。
- **重复检测合并**(F-05)—— 同 URL 标出重复,一键合并(保留最近活跃的)。
- **全局搜索**(`⌘⇧K`)—— fuse.js 模糊匹配打开与已归档标签,`↵` 直达、`⌘↵` 恢复整个任务。未输入时以启动器形式展示你最近使用与 ★ 重点标签。
...
- **GitHub / Bitbucket 元数据**(F-09)—— GitHub PR/Issue 与 Bitbucket Cloud PR/Issue 的标签行显示「类型 + 编号」等宽徽章(`PR #482` / `#212`),悬停显示 `owner/repo`(Bitbucket 为 `workspace/repo`);GitHub 还会剥掉冗长标题尾部,只留真正标题。纯 URL 解析,零请求零权限。
...
- **导出与导入**(F-12)—— 任务一键导出为 Markdown(标题 + 链接,复制到剪贴板,贴周报 / Notion);设置里「导出全部数据 (JSON)」备份、「导入 JSON」还原。导入为**增量、非破坏式** —— 导入的任务落入「已归档」,绝不覆盖现有数据,可安全用于跨机器备份 / 迁移。
...
4. 建议把工具栏图标 **固定**;点它即可打开侧边栏(或用快捷键 `⌘⇧O` / `Ctrl+Shift+O` 开面板、`⌘⇧K` / `Ctrl+Shift+K` 唤起搜索)。
...
**数据与备份**:所有任务 / 标签存于浏览器本地 IndexedDB,不上传。设置里「导出全部数据 (JSON)」可随时备份,「导入 JSON」可还原或迁移到另一台机器(导入的任务落入「已归档」,不覆盖任何现有数据)。
