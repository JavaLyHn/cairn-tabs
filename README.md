...
MVP 核心闭环(F-01~F-06)与 v1.1 程序员特化(F-07~F-12)已实现并跑通:
...
- **自动聚簇引擎**(F-07):基于 opener 链 / 时间窗 / 域名打分,把新标签归入合适的簇;未分类里的 opener 树(≥3、15min 内)自动升格为新簇;**同域成簇建议**——未分类里同一站点(eTLD+1)标签攒够阈值(默认 4,设置里可调 2–8)时,顶部出现「成簇」建议,确认即拎成一个新簇(弱信号只建议不自动,区别于 opener 树的自动升格);从纠正中学习(拖出记负样本、人工归属锁定)。保守优先,宁进未分类不误归。**可在设置里一键关闭**(关闭后新标签只进「未分类」,不自动建组)
...
- **陈旧检测**(F-10):超过阈值天数(默认 7 天)未访问的打开标签,从各任务抽出、集中到底部灰暗下沉簇,一键「全部归档」(可撤销)。不弹通知,可在设置里关闭
- **标签挂起与内存回收**(F-11):**默认关闭**;开启后每 5 分钟扫描,挂起空闲超阈值(默认 30 分钟)、非活跃/音频/置顶且非 localhost 的标签,释放内存(点击自动重载),底部状态栏显示累计估算回收量。localhost 白名单护 dev server
