...
- **自动聚簇引擎**(F-07):基于 opener 链 / 时间窗 / 域名打分,把新标签归入合适的簇;未分类里的 opener 树(≥3、15min 内)自动升格为新簇;从纠正中学习(拖出记负样本、人工归属锁定)。保守优先,宁进未分类不误归。**可在设置里一键关闭**(关闭后新标签只进「未分类」,不自动建组)
...
尚未实现(见设计文档 Roadmap):自动聚簇的「同域升格」、AI 命名与归类(F-13,BYO Key,v1.5)。
